    ...
    return (
        <div className='w-full'>
            ...
            <div className="flex mt-4 left-0 md:px-0 bg-indigo-950 z-50 h-8 relative">
                ...
                <div className={`md:flex absolute md:static md:mt-0 mt-8 mx-2 md:mx-24 bg-indigo-950 md:z-auto z-[-1] left-0 w-48 md:w-auto md:pl-0 pl-6 transition-all duration-500 ease-in ${open ? '-left-4 ':'left-[-250px]'}`}>
                {
  Links.map((link) => (
    <div
      key={link.name}
    ...
    >
      ...
      >
        <li
          className={`px-2 py-1 hover:text-teal-400 list-none duration-500 ${
            location.pathname === link.link ? "text-teal-500" : "text-white"
          }`}
        >
          {link.name}
        </li>
      ...
      {link.name === "Publications" && showSubmenu && (
        <ul className="absolute left-0 top-8 z-10 w-32 px-2 bg-white shadow py-1"> {/* Add necessary styling */}
          {link.submenu.map((submenuLink) => (
            <li key={submenuLink.name}>
              <Link className='hover:text-teal-400' to={submenuLink.link}>{submenuLink.name}</Link>
            </li>
          ))}
        </ul>
      )}
